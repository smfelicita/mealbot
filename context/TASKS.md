...
- [x] Страница "Блюда" (DishesPage): поиск, фильтр-чипы, FAB добавить рецепт, SVG иконки
...
- [x] Комментарии к рецептам: модель Comment (БД), API /api/comments, UI в DishDetailPage
...
- [ ] Анализ нагрузки и ресурсов: CPU/RAM VPS, DB connections, запросы в пиках
- [ ] Анализ API-затрат: стоимость Claude API, подсчёт токенов на запрос, прогноз при росте пользователей

### Дизайн-система / фронтенд
- [ ] TelegramAuthPage — переписать со старых CSS-переменных на Tailwind (легаси `var(--text2)`, `auth-page`)
- [ ] Произвольные размеры шрифта — заменить `text-[11px]`, `text-[13px]`, `text-[15px]`, `text-[16px]`, `text-[17px]`, `text-[22px]`, `text-[24px]`, `text-[26px]` на именованные токены в tailwind.config
